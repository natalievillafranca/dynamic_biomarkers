...
golist=read.table("BP_unmerged_hostGO_MM_purple_numericASV.csv",sep="	",header=T) #read in proper GO list from gomwu output - BP/MF/CC and module name
in.mwu=read.table("MWU_BP_unmerged_hostGO_MM_purple_numericASV.csv",sep=" ",header=T)
...
tukey_results <- IRF2HubTuk$`final$genotype`
...
tukey_results <- IRF2otherTuk$`final$genotype`
pvals <- tukey_results[, "p adj"]
names(pvals) <- rownames(tukey_results)
