...
dfull=read.csv("unmerged_hostGO_MM_magenta_ASV.csv") #read in proper VSD file for genes in module - generated in wgcna script 
...
golist=read.table("BP_unmerged_hostGO_MM_magenta_ASV.csv",sep="	",header=T) #read in proper GO list from gomwu output - BP/MF/CC and module name
in.mwu=read.table("MWU_BP_unmerged_hostGO_MM_magenta_ASV.csv",sep=" ",header=T)
d=read.csv("host_vsd_MM_asvmagenta.csv")#read in proper VSD file for genes in module - generated in wgcna script 
...
datTraits=datTraits%>%relocate("34", .after ="44")
datTraits=datTraits%>%relocate("7", .after ="34")
